...
class SubscriptionDashboardViewModel extends InsiteViewModel {
  ...
  getSubscriptionDashboardData() async {
    if (enableGraphQl) {
      ...
      Logger().i("dashboardData:${data!.toJson()}");

      if (data == null) {
        Logger().d('no results found');
        _loading = false;
      } else {
        final totalDeviceSupplied = data.totalDevicesSupplied;
        final plantAssetCount = data.plantAssetCount;
        final activeSubScription = data.activeSubscription;
        final yetToBeActivated = data.yetToBeActivated;
        final subScriptionEnded = data.subscriptionEnded;
        final subScriptionEndingThisMOnth = data.assetActivationByMonth;
        ...
        if (data.modelFleetList!.isNotEmpty) {
          ...
          for (var item in items!) {
            ...
            _modelCount.add(item.modelCount);
          }
        }
        Logger().wtf(modelNames);
